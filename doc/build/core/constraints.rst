...
is specified by constructing a :class:`~sqlalchemy.ForeignKey` object
as an argument to a :class:`~sqlalchemy.Column` object::
...
The argument to :class:`~sqlalchemy.ForeignKey` is most commonly a
...
also be an actual :class:`~sqlalchemy.Column` object, which as we'll
see later is accessed from an existing :class:`~sqlalchemy.Table`
...
:class:`~sqlalchemy.ForeignKeyConstraint` object. This object can
...
:class:`~sqlalchemy.ForeignKeyConstraint` is the only way to define a
composite foreign key. While we could also have placed individual
:class:`~sqlalchemy.ForeignKey` objects on both the
...
:class:`~sqlalchemy.ForeignKey` and
:class:`~sqlalchemy.ForeignKeyConstraint` objects support the
...
``unique`` keyword on :class:`~sqlalchemy.Column`. Explicitly named
unique constraints and/or those with multiple columns are created via the
:class:`~sqlalchemy.UniqueConstraint` table-level construct.
...
Table level, using the :class:`~sqlalchemy.CheckConstraint` construct.
...
:class:`~sqlalchemy.Column`, which also modifies the usage of
...
more than one column, use the :class:`~sqlalchemy.Index` construct,
which requires a name.

Below we illustrate a :class:`~sqlalchemy.Table` with several
:class:`~sqlalchemy.Index` objects associated. The DDL for "CREATE
...
The :class:`~sqlalchemy.Index` object also supports its own ``create()`` method:
